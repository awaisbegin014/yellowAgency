...
export default async function CaseStudyPage({ params }: PageProps<"/case-studies/[slug]">) {
  ...
  return <main id="top">
    <section className="page-hero page-hero--yellow"><div className="container"><div className="article-header"><p className="eyebrow">{study.industry} case study / {study.client}</p><h1>{study.title}</h1><p className="page-hero__lead">{study.summary}</p></div></div></section>
    ...
    <section className="section section--soft"><div className="container detail-outcomes"><div><p className="eyebrow">The result</p><div className="mini-metrics"><div><strong>{study.metric}</strong><span>{study.result}</span></div></div></div><div><h2>One connected system changed the trajectory.</h2><p className="large-copy">Yellow aligned the customer story, acquisition plan, website experience, and follow-up around one measurable growth goal.</p><p>These are placeholder case-study details ready to be replaced by approved client evidence, final statistics, quotes, and campaign visuals.</p></div></div></section>
    <section className="section"><div className="container"><div className="section-heading"><div><p className="eyebrow">The work</p><h2>A campaign designed as one experience.</h2></div><p>Every touchpoint was built to reinforce the same useful promise and make the next step feel natural.</p></div><div className="collection-grid">{["Strategy and positioning", "Campaign creative", "Digital experience"].map((item) => <article key={item} className="collection-card"><MediaPlaceholder label={`${item} image placeholder`} /><h3>{item}</h3></article>)}</div></div></section>
