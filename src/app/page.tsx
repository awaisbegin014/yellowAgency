...
function MetricCard({ value, label, offset }: { value: string; label: string; offset: number }) {
  return <article className="hc-metric-card"><b>{value}</b><span>{label}</span><div className="hc-metric-card__mountain" style={{ clipPath: `polygon(0 ${70 - offset}%, 14% ${78 - offset}%, 28% ${61 + offset}%, 43% 72%, 58% ${53 + offset}%, 72% 68%, 87% ${45 + offset}%, 100% 58%, 100% 100%, 0 100%)` }} /></article>;
}
...
export default function Home() {
  return (
    <main id="top" className="reference-home">
      ...
      <section className="hc-section hc-dental"><div className="hc-shell hc-centered"><h2>How We Increased Organic Leads For Our Dental Client?</h2><p>Partnering with a dental practice, we built a digital marketing strategy around SEO, PPC, and targeted social campaigns. The result was a dramatic improvement in organic visibility, qualified leads, and booked patients.</p><div className="hc-toggle"><span>Before Yellow</span><i /><span>After Yellow</span></div><div className="hc-metrics"><MetricCard value="90%" label="Organic Traffic" offset={8} /><MetricCard value="30%" label="Leads" offset={2} /><MetricCard value="120%" label="Customers" offset={12} /></div></div></section>

      <section className="hc-section hc-partner-model"><div className="hc-shell"><div className="hc-split-heading"><h2>What Our White Label<br />Partners Have To Say</h2><p>See the difference we make through our partners&apos; eyes. Read their stories and learn how our digital marketing solutions have helped boost their success.</p></div><div className="hc-model-cards"><article><h3>Yellow</h3><ul>{serviceNames.slice(0, 8).map((item) => <li key={item}>{item}</li>)}</ul></article><article><h3>You As Our Partner</h3><ul><li>Focus on getting more business</li><li>Drive sales</li><li>Ensure client satisfaction</li><li>Oversee project success</li></ul></article><article className="is-yellow"><div className="hc-avatar"><Image src="/images/unsplash/portrait-04.jpg" alt="Yellow agency partner" fill sizes="90px" /></div><h3>Josh Radford</h3><span>Agency Partner</span><blockquote>“I can call ourselves a marketing agency after having Yellow behind me.”</blockquote></article></div></div></section>
