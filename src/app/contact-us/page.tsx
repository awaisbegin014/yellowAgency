    ...
    <main id="top" className="yellow-contact">
      <section className="yellow-contact__hero">
        <StaggerGroup className="container">
          <StaggerItem as="p" className="eyebrow">Let&apos;s grow your agency together</StaggerItem>
          <StaggerItem as="h1">Bring us the challenge. <span>We&apos;ll make the next move clear.</span></StaggerItem>
          <StaggerItem as="p">From a single client project to a complete white-label delivery partnership, Yellow gives your agency a reliable, proactive, and transparent team behind the scenes.</StaggerItem>
          <StaggerItem className="yellow-contact__hero-actions">
          ...
          <StaggerGroup className="yellow-contact__trust-row" aria-label="How Yellow works">
            {['Reliable delivery', 'Secure collaboration', 'Proactive support', 'Transparent reporting'].map((item, index) => (
              <StaggerItem as="span" key={item}><b>{String(index + 1).padStart(2, '0')}</b>{item}</StaggerItem>
            ))}
          </StaggerGroup>
        </StaggerGroup>
      </section>
      ...
      <section className="yellow-contact__sales" id="contact-sales">
        <div className="container yellow-contact__sales-grid">
          <Reveal className="yellow-contact__sales-copy">
          ...
          <Reveal delay={0.15}><ContactSalesForm /></Reveal>
        </div>
      </section>

      <section className="yellow-contact__services">
        <div className="container">
          <Reveal className="yellow-contact__section-heading">
      ...
      <section className="yellow-contact__direction">
        <div className="container yellow-contact__direction-grid">
          <Reveal className="yellow-contact__direction-copy">
          ...
          <StaggerGroup className="yellow-contact__principles">
            <StaggerItem as="article"><span>Our vision</span><h3>Make expert digital delivery accessible to every ambitious agency.</h3><p>A flexible specialist team should feel like a growth advantage—not another layer to manage.</p></StaggerItem>
            <StaggerItem as="article"><span>Our mission</span><h3>Carry the fulfillment load so agency leaders can focus on relationships and growth.</h3><p>Clear processes, useful reporting, and accountable specialists keep every project moving.</p></StaggerItem>
          </StaggerGroup>
        </div>
      </section>
