...
function AnalyticsMontage() {
  return (
    <div className="about-reference__analytics" aria-label="Yellow marketing analytics and campaign dashboard illustration">
      ...
      <FloatLoop className="about-reference__analytics-card about-reference__analytics-card--left" duration={5}>
        <span>Audience growth</span><b>+40.94%</b><i />
      </FloatLoop>
      <FloatLoop className="about-reference__analytics-card about-reference__analytics-card--right" delay={0.6} duration={5.5}>
  ...
  return (
    <main id="top" className="inner-page about-reference">
      <section className="about-reference__hero">
        <StaggerGroup className="container about-reference__hero-inner">
          <StaggerItem as="p" className="eyebrow">The team behind your team</StaggerItem>
          <StaggerItem><AccentHeading as="h1">About Yellow Clicks</AccentHeading></StaggerItem>
          <StaggerItem as="p">Yellow is a full-stack white-label digital marketing agency built for ambitious agencies that need specialist depth without adding another department. You own every client relationship; our team delivers the work behind your brand.</StaggerItem>
      ...
      <section className="section about-reference__overview">
        <div className="container">
          <Reveal className="about-reference__section-copy">
          ...
          <Reveal delay={0.15}><AnalyticsMontage /></Reveal>
        </div>
      </section>

      <section className="section about-reference__team">
        <div className="container">
          <Reveal className="about-reference__centered-heading"><p className="eyebrow">The people</p><AccentHeading>The management team.</AccentHeading><p>Senior guidance stays close to the work, while specialist delivery teams bring every plan to life.</p></Reveal>
          <StaggerGroup className="about-reference__team-grid">
            {team.map((member) => (
              <StaggerItem as="article" key={member.name}>
      ...
      <section className="section about-reference__connected">
        <Reveal className="container about-reference__connected-grid">
          <AgencyCollage />
          <div>
      ...
      <section className="section about-reference__services" id="about-services">
        <div className="container">
          <Reveal className="about-reference__centered-heading"><p className="eyebrow">Our services</p><AccentHeading>Complete digital delivery under your name.</AccentHeading><p>Choose a service to see the specialist dashboard and the work Yellow manages behind your brand.</p></Reveal>
          <ServiceTabs />
        </div>
      </section>
      ...
      <section className="section about-reference__vision">
        <div className="container">
          <Reveal className="about-reference__centered-heading"><p className="eyebrow">What keeps us focused</p><AccentHeading>Our vision and mission.</AccentHeading><p>Growth matters most when the process is clear, the work is useful, and every partner feels supported.</p></Reveal>
          <Reveal delay={0.1} className="about-reference__vision-grid">
      ...
      <section className="section about-reference__contact">
        <Reveal className="container about-reference__contact-grid">
          <div>
      ...
      <section className="section about-reference__onboarding">
        <div className="container">
          <Reveal className="about-reference__centered-heading"><p className="eyebrow">Simple from the start</p><AccentHeading>White-label onboarding process.</AccentHeading><p>A secure, practical five-step process keeps responsibilities clear and gets your delivery team moving quickly. Before production begins, we align goals, platforms, ownership, approval stages, and reporting expectations with your agency. You leave onboarding with a documented launch plan, a dedicated point of contact, and complete visibility into what happens next.</p></Reveal>
          <StaggerGroup className="about-reference__onboarding-flow">
            {onboarding.map(([number, title, text]) => <StaggerItem as="article" key={number}><AboutHexagon><b>{number}</b></AboutHexagon><i /><h3>{title}</h3><p>{text}</p></StaggerItem>)}
          </StaggerGroup>
          <StaggerGroup className="about-reference__onboarding-support">
            <StaggerItem as="article"><span>01</span><div><h3>One shared workspace</h3><p>Assets, access details, priorities, and approvals stay organized for both teams.</p></div></StaggerItem>
            <StaggerItem as="article"><span>02</span><div><h3>Visible weekly progress</h3><p>Your agency receives concise updates on completed work, active tasks, and next actions.</p></div></StaggerItem>
            <StaggerItem as="article"><span>03</span><div><h3>Clear approval points</h3><p>Nothing client-facing moves forward until the responsible agency stakeholder signs off.</p></div></StaggerItem>
          </StaggerGroup>
        </div>
      </section>

      <section className="about-reference__cta">
        <Reveal className="container"><div><p className="eyebrow">A quiet team behind your brand</p><h2>Can a third-party agency truly take care of your clients?</h2><p>Book a free conversation and see how our systems, specialists, and white-label safeguards support confident agency growth.</p></div><Link href="/book-appointment" className="button button--dark">Book a free strategy call <span aria-hidden="true">↗</span></Link></Reveal>
      </section>
    </main>
  );
}
