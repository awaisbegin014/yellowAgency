...
export function GrowthCaseStudyPage({ slug }: { slug: GrowthCaseStudySlug }) {
  ...
  return (
    <main id="top" className={`inner-page growth-study growth-study--${slug}`}>
      <section className="growth-study__hero"><StaggerGroup className="container"><StaggerItem as="p" className="eyebrow">{study.eyebrow}</StaggerItem><StaggerItem as="h1">{study.headlineStart}<span>{study.headlineAccent}</span>{study.headlineEnd}</StaggerItem><StaggerItem><Link href="/book-appointment" className="button button--dark">Free 20-min consultation <span aria-hidden="true">↗</span></Link></StaggerItem></StaggerGroup></section>

      <section className="growth-study__section growth-study__overview"><div className="container growth-study__overview-grid"><Reveal><p className="eyebrow">Client overview</p><StudyHeading>A strong business ready for a stronger acquisition system.</StudyHeading><p>{study.clientOverview}</p></Reveal><StaggerGroup className="growth-study__briefs"><StaggerItem as="article"><span>01</span><h3>Their objective</h3><p>{study.objective}</p></StaggerItem><StaggerItem as="article"><span>02</span><h3>The challenge</h3><p>{study.challenge}</p></StaggerItem></StaggerGroup></div></section>

      <section className="growth-study__section growth-study__solution"><Reveal className="container growth-study__copy"><p className="eyebrow">Our digital marketing solution</p><StudyHeading>{study.solutionHeading}</StudyHeading><div className="growth-study__solution-copy">{study.solution.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div><MediaPlaceholder label={study.mediaLabel} src={study.mediaImage} className="growth-study__media" /></Reveal></section>

      <section className="growth-study__section growth-study__method"><div className="container"><Reveal className="growth-study__section-heading"><div><p className="eyebrow">How we did it</p><StudyHeading>{study.methodHeading}</StudyHeading></div><p>{study.methodIntro}</p></Reveal><StaggerGroup className="growth-study__method-grid">{study.methods.map((method, index) => <StaggerItem as="article" key={method}><span>{String(index + 1).padStart(2, "0")}</span><h3>{method}</h3></StaggerItem>)}</StaggerGroup></div></section>

      <section className="growth-study__section growth-study__lead-magnet"><Reveal className="container growth-study__lead-grid"><div><p className="eyebrow">Free lead-magnet campaign</p><StudyHeading>Turn useful expertise into measurable demand.</StudyHeading><p>Helpful content creates a reason for the right prospect to engage before they are ready to buy. Yellow connects that value to focused promotion and clear follow-up.</p><ul>{sharedLeadBenefits.map((benefit) => <li key={benefit}>{benefit}</li>)}</ul><Link href="/book-appointment" className="button button--dark">Plan your campaign <span aria-hidden="true">↗</span></Link></div><div className="growth-study__lead-card"><span>Free growth playbook</span><strong>{study.metrics[0][0]}</strong><p>{study.metrics[0][1]}</p><div><i /><i /><i /></div><small>Strategy · Media · Conversion</small></div></Reveal></section>

      <section id="results" className="growth-study__section growth-study__results"><div className="container"><Reveal className="growth-study__section-heading growth-study__results-heading"><div><p className="eyebrow">The results</p><StudyHeading>{study.resultHeading}</StudyHeading><div className="growth-study__results-copy">{study.resultCopy.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div></Reveal><StaggerGroup className="growth-study__metric-grid">{study.metrics.map(([value, label]) => <StaggerItem as="article" key={label}><strong>{value}</strong><span>{label}</span></StaggerItem>)}</StaggerGroup><Reveal><PerformanceBoard study={study} /></Reveal></div></section>

      <section className="growth-study__section growth-study__analysis"><Reveal className="container growth-study__copy"><p className="eyebrow">Before + after analysis</p><StudyHeading>See what changed—not just what increased.</StudyHeading><p>Better results came from fixing the system behind the numbers: targeting, message, journey, and measurement.</p><BeforeAfter study={study} /></Reveal></section>

      <section className="growth-study__section growth-study__conclusion"><Reveal className="container growth-study__conclusion-grid"><div><p className="eyebrow">Spend less and earn more</p><StudyHeading>{study.conclusionHeading}</StudyHeading>{study.conclusion.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div><blockquote><span aria-hidden="true">“</span><p>I couldn&apos;t recommend the team more highly. Their work is impeccable and their communication is incredibly professional.</p><footer><b>SB</b><div><strong>Seth Brown</strong><small>Agency partner</small></div><em aria-label="Five out of five stars">★★★★★</em></footer></blockquote></Reveal></section>

      <section className="growth-study__section growth-study__onboarding"><Reveal className="container growth-study__copy"><p className="eyebrow">White-label onboarding</p><StudyHeading>A simple, confidential path from opportunity to delivery.</StudyHeading><p>Your agency remains at the center of the client relationship. Yellow brings the audit, strategy, specialists, and reporting structure that make confident delivery possible.</p><OnboardingFlow /></Reveal></section>

      <section className="growth-study__cta"><Reveal className="container"><div><p className="eyebrow">Your next growth story</p><h2>Can Yellow take care of your clients?</h2><p>Book a focused strategy call and see how our white-label team can support your agency&apos;s next result.</p></div><Link href="/book-appointment" className="button button--dark">Book a free strategy call <span aria-hidden="true">↗</span></Link></Reveal></section>
    </main>
  );
}
